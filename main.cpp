...
int main() 
{
    ...
    int n, m;
    ifstream agentUtilities("utilities.txt");
    agentUtilities >> n >> m;
    ...
    while( (minBundlePrice < EFMaxBundlePrice) && floatIsEqual(minBundlePrice, EFMaxBundlePrice, EPS)==false ) {
        ...
        while(1) {
            ...
            if(pathViolater!=-1) {
                cout << "----> Transferring item to Agent " << predAgentToItem[itemViolater];
            ...
            if(q.empty() && leastSpenders.size()>1) {
